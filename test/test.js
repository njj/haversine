...
suite('haversine', function(){
    ...
    for (const [start, end, expected, options] of tests) {
        test(`it should return ${expected} for ${start} .. ${end}`, () => {
            assert.ok(Math.abs(haversine(start, end, options) - expected) < 0.001)
        })
    }
