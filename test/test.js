  ...
  tests.forEach(function (t, i) {
    if (i % 2 === 0) {
      test('it should return ' + t[2] + ' mi for ' + t[0] + ' .. ' + t[1], function () {
        assert.equal(Math.abs((haversine.distance(t[0], t[1], Object.assign({ unit: 'mile' }, t[3])) - t[2]) / t[2]).toFixed(2), "0.00")
      })
    } else {
      test('it should return ' + t[2] + ' km for ' + t[0] + ' .. ' + t[1], function () {
        assert.equal(Math.abs((haversine.distance(t[0], t[1], Object.assign({}, t[3])) - t[2]) / t[2]).toFixed(2), "0.00")
      })
    }
  })

  test('it should return false that distance is within 1 mi threshold', function () {
    assert.equal(false, haversine.withinDistance(tests[0], tests[1], 1))
  })

  test('it should return false that distance is within 1 km threshold', function () {
    assert.equal(false, haversine.withinDistance(tests[1], tests[1], 1, { threshold: 1, unit: 'km' }))
  })
})
